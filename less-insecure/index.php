<?php
    ...
    function checkCredentials($username, $password){
        ...
        $sql = "SELECT * FROM csis2440_user WHERE username = '$username';";
        ...
        if($row != Null && $row['password'] == $password){
            $sql = "UPDATE csis2440_user SET `loginCount` = loginCount+1 WHERE `id` = '".$row['id']."'";
    ...
    function changePassword($username, $password){
        ...
        if($row != Null){
            $sql = "UPDATE csis2440_user SET `password` = '$password' WHERE `id` = '".$row['id']."'";
            mysqli_query($conn, $sql);
            return true;
        }
